...
function publicPeerUrl(node) {
  const explicit = process.env.P2P_PUBLIC_URL || process.env.VITE_P2P_PUBLIC_URL;
  if (explicit) return explicit;
  return `ws://${firstLanAddress()}:${node.port}`;
}

function sendBootstrapRegister(node) {
  if (bootstrapSocket?.readyState !== WebSocket.OPEN) return;
  const url = publicPeerUrl(node);
  node.publicUrl = url;
  bootstrapSocket.send(JSON.stringify({ type: 'peer:register', peerId: node.peerId, url, wallet: activeWallet() || null }));
  console.log('[bootstrap] registered', node.peerId, url);
}

function connectBootstrap(node) {
  if (!BOOTSTRAP_URL) return;
  if (bootstrapSocket?.readyState === WebSocket.OPEN) {
    sendBootstrapRegister(node);
    return;
  }
  try {
    ...
    bootstrapSocket.on('open', () => {
      sendBootstrapRegister(node);
      if (bootstrapHeartbeat) clearInterval(bootstrapHeartbeat);
      bootstrapHeartbeat = setInterval(() => {
        if (bootstrapSocket?.readyState === WebSocket.OPEN) {
          sendBootstrapRegister(node);
          bootstrapSocket.send(JSON.stringify({ type: 'peer:heartbeat', peerId: node.peerId, url: publicPeerUrl(node), wallet: activeWallet() || null }));
        }
      }, 30000);
    });
    bootstrapSocket.on('message', (raw) => {
      try {
        const msg = JSON.parse(raw.toString());
        const peers = msg.type === 'bootstrap:peers' ? msg.peers : msg.type === 'bootstrap:new-peer' ? [msg.peer] : [];
        for (const peer of peers || []) {
          if (!peer?.peerId || !peer?.url || peer.peerId === node.peerId) continue;
          if (peer.url.includes('127.0.0.1') || peer.url.includes('localhost')) continue;
          try { node.connectPeer({ peerId: peer.peerId, url: peer.url }); } catch (e) { console.warn('[bootstrap] peer connect failed:', e?.message || e); }
        }
      } catch (e) { console.warn('[bootstrap] bad message:', e?.message || e); }
    });
    bootstrapSocket.on('close', () => {
      if (bootstrapHeartbeat) clearInterval(bootstrapHeartbeat);
      bootstrapSocket = null;
      setTimeout(() => connectBootstrap(node), 5000);
    });
...
function chunkStoreDir() {
  return process.env.P2P_CHUNK_STORE_DIR || path.join(app.getPath('userData'), 'native-p2p-storage', 'chunks');
}
...
function walletSummary() { const plan = PLANS[walletState.planId] || PLANS.free; const usedBytes = walletState.connected ? totalStoredBytesForWallet() : 0; return { ok: true, ...walletState, address: activeWallet() || walletState.address, plan, plans: Object.values(PLANS), usedBytes, remainingBytes: Math.max(0, plan.quotaBytes - usedBytes), paymentRequired: PLANS[walletState.planId]?.locked && !walletState.paidUntil }; }
function assertWalletUploadAllowed(nextBytes = 0) { assertVerifiedWallet(); const plan = PLANS[walletState.planId] || PLANS.free; if (plan.locked && !walletState.paidUntil) throw new Error('Payment required. Paid plans are locked until payment verification is connected.'); if (totalStoredBytesForWallet() + nextBytes > plan.quotaBytes) throw new Error(`Storage quota exceeded. Current plan: ${plan.name}.`); }
...
function preferredReplicaTargets(node, currentReplicas = []) {
  const excluded = new Set([node.peerId, ...currentReplicas]);
  const peers = Array.from(node.peerInfo.values())
    .filter((peer) => node.peerSockets.get(peer.peerId)?.readyState === WebSocket.OPEN)
    .sort((a, b) => {
      const aw = String(a.peerId || '').startsWith('aws-storage-peer') || String(a.url || '').includes('54.166.171.208') ? 1 : 0;
      const bw = String(b.peerId || '').startsWith('aws-storage-peer') || String(b.url || '').includes('54.166.171.208') ? 1 : 0;
      return bw - aw;
    })
    .map((peer) => peer.peerId)
    .filter((peerId) => !excluded.has(peerId));
  return peers.slice(0, Math.max(0, TARGET_REPLICAS - new Set([node.peerId, ...currentReplicas]).size));
}

async function replicateChunkIfPossible(node, chunkMeta, manifest) {
  const chunk = node.getLocalChunk?.(chunkMeta.hash) || node.localChunks.get(chunkMeta.hash);
  if (!chunk) return { ok: false, reason: 'missing-local-chunk' };
  const replicas = new Set([node.peerId, ...(chunkMeta.replicas || [])]);
  if (replicas.size >= TARGET_REPLICAS) return { ok: true, repaired: false, replicas: Array.from(replicas) };
  const targets = preferredReplicaTargets(node, Array.from(replicas));
  if (!targets.length) return { ok: false, reason: 'no-targets', replicas: Array.from(replicas) };
  try {
    const result = node.putChunkOnNetwork(chunk, targets);
    for (const peerId of result.replicas || []) replicas.add(peerId);
    chunkMeta.replicas = Array.from(replicas);
    manifest.replicas = Array.from(new Set([...(manifest.replicas || []), ...chunkMeta.replicas]));
    return { ok: true, repaired: true, targets, replicas: chunkMeta.replicas };
  } catch (error) {
    return { ok: false, reason: error?.message || 'replication-failed', replicas: Array.from(replicas) };
  }
}

async function runAutoRepair({ manual = false } = {}) {
  if (autoRepairRunning) return { ok: true, skipped: true, reason: 'already-running' };
  autoRepairRunning = true;
  try {
    const node = ensureTransport({});
    connectBootstrap(node);
    if (walletState.connected) await syncPull();
    const files = walletManifests();
    let repairedChunks = 0;
    let missingLocalChunks = 0;
    let changed = false;
    for (const manifest of files) {
      for (const chunk of manifest.chunks || []) {
        const result = await replicateChunkIfPossible(node, chunk, manifest);
        if (result.reason === 'missing-local-chunk') missingLocalChunks++;
        if (result.repaired) { repairedChunks++; changed = true; }
      }
      if (changed) await syncPush(manifest);
    }
    if (changed) persistManifests();
    const report = { ok: true, manual, files: files.length, repairedChunks, missingLocalChunks, peers: node.connectedPeerIds().length, at: new Date().toISOString() };
    if (manual || repairedChunks || missingLocalChunks) console.log('[auto-repair]', report);
    return report;
  } finally {
...
function startAutoRepairLoop() {
  ...
  autoRepairTimer = setInterval(() => {
    runAutoRepair().catch((error) => console.warn('[auto-repair] failed:', error?.message || error));
  }, AUTO_REPAIR_INTERVAL_MS);
  console.log('[auto-repair] loop enabled every', AUTO_REPAIR_INTERVAL_MS, 'ms');
}
