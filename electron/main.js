...
const ENCRYPTION_KEY_SOURCE = 'wallet-address-stable-v2';
...
function stableWalletSecret(ownerWallet = activeWallet()) {
  ...
  return crypto.createHash('sha256').update(`p2p.cloud:${ENCRYPTION_KEY_SOURCE}:${wallet}`).digest('base64');
}
function encryptionKeyForWallet(ownerWallet = activeWallet()) {
  return crypto.createHash('sha256').update(`${normalizeWallet(ownerWallet)}:${stableWalletSecret(ownerWallet)}`).digest();
}
function encryptPrivateBuffer(plainBuffer, ownerWallet = activeWallet()) {
  const iv = crypto.randomBytes(12);
  const key = encryptionKeyForWallet(ownerWallet);
  const cipher = crypto.createCipheriv(ENCRYPTION_ALGORITHM, key, iv);
  const ciphertext = Buffer.concat([cipher.update(plainBuffer), cipher.final()]);
  return { ciphertext, encryption: { version: 3, algorithm: ENCRYPTION_ALGORITHM, keySource: ENCRYPTION_KEY_SOURCE, iv: iv.toString('base64'), authTag: cipher.getAuthTag().toString('base64'), originalHash: hashBufferHex(plainBuffer), originalSize: plainBuffer.length } };
}
function decryptPrivateBuffer(ciphertext, manifest) {
  ...
  if (manifest.encryption.keySource && manifest.encryption.keySource !== ENCRYPTION_KEY_SOURCE) throw new Error(`This file was encrypted with an older key source (${manifest.encryption.keySource}). Re-upload it once using the current app.`);
  const key = encryptionKeyForWallet(manifest.ownerWallet);
...
async function syncPull() { try { if (!isManifestSyncEnabled() || !walletState.connected || !walletState.address) return; const remote = await pullWalletManifests(activeWallet()); if (!Array.isArray(remote)) return; const map = new Map(manifests.map((m) => [`${normalizeWallet(m.ownerWallet)}:${m.hash}`, m])); for (const m of remote) map.set(`${normalizeWallet(m.ownerWallet)}:${m.hash}`, { ...m, ownerWallet: normalizeWallet(m.ownerWallet) }); manifests = Array.from(map.values()); persistManifests(); } catch (e) { console.warn('[manifest-sync] pull failed:', e?.message || e); } }
...
ipcMain.handle('p2p:upload', async (_event, payload = {}) => { const node = ensureTransport({}); if (!payload.bytes) throw new Error('File bytes are required'); const originalBuffer = Buffer.from(payload.bytes); assertWalletUploadAllowed(originalBuffer.length); const ownerWallet = activeWallet(); const privateFile = Boolean(payload.isEncrypted); const secured = privateFile ? encryptPrivateBuffer(originalBuffer, ownerWallet) : { ciphertext: originalBuffer, encryption: null }; const storedBuffer = secured.ciphertext; const chunks = splitIntoChunks(storedBuffer); const tree = buildMerkleTree(chunks.map((c) => c.hash)); const storedHash = hashBufferHex(storedBuffer); const manifest = { id: `${ownerWallet}:${storedHash}`, name: String(payload.name || 'file'), size: originalBuffer.length, storedSize: storedBuffer.length, hash: storedHash, rootHash: tree.root, uploadedAt: new Date().toISOString(), isEncrypted: privateFile, encryption: secured.encryption, mimeType: payload.mimeType ? String(payload.mimeType) : 'application/octet-stream', chunkSize: CHUNK_SIZE_BYTES, totalChunks: chunks.length, ownerNodeId: node.peerId, ownerWallet, planId: walletState.planId, replicas: [node.peerId], chunks: [] };
  for (const chunk of chunks) { const chunkPayload = { hash: chunk.hash, data: chunk.data.toString('base64'), index: chunk.index, size: chunk.size, ownerWallet, encrypted: privateFile }; node.storeLocalChunk?.(chunkPayload); manifest.chunks.push({ index: chunk.index, hash: chunk.hash, size: chunk.size, replicas: [node.peerId], proof: getMerkleProof(tree, chunk.index) }); }
  manifests = manifests.filter((m) => !(normalizeWallet(m.ownerWallet) === ownerWallet && m.hash === manifest.hash)); manifests.push(manifest); persistManifests(); persistWallet(); await syncPush(manifest); return { ok: true, file: manifest, summary: networkSummary() }; });
ipcMain.handle('p2p:download', async (_event, payload = {}) => { assertVerifiedWallet(); const node = ensureTransport({}); const manifest = findManifest(payload); if (!manifest) throw new Error('File not found for this wallet'); const buffers = []; for (const meta of [...(manifest.chunks || [])].sort((a, b) => a.index - b.index)) { const local = node.getLocalChunk?.(meta.hash) || node.localChunks?.get(meta.hash); const chunk = local || await node.fetchChunkFromNetwork(meta.hash); node.storeLocalChunk?.(chunk); const buffer = Buffer.from(chunk.data, 'base64'); if (hashBufferHex(buffer) !== meta.hash) throw new Error(`Chunk integrity failed: ${meta.hash}`); buffers.push(buffer); } const storedBuffer = Buffer.concat(buffers); if (hashBufferHex(storedBuffer) !== manifest.hash) throw new Error('File integrity failed'); const outputBuffer = manifest.isEncrypted ? decryptPrivateBuffer(storedBuffer, manifest) : storedBuffer; return { ok: true, file: manifest, bytes: Array.from(outputBuffer) }; });
...
ipcMain.handle('p2p:repair', async () => ({ ok: true, report: [] }));
