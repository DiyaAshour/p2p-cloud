...
function resolvePreloadPath() {
  ...
  if (!fs.existsSync(preloadPath)) {
    throw new Error(`Missing Electron preload file: ${preloadPath}`);
  }
...
function createMainWindow() {
  ...
  mainWindow.once('ready-to-show', () => {
    mainWindow?.show();
    if (IS_DEV || process.env.ELECTRON_OPEN_DEVTOOLS === '1') mainWindow?.webContents.openDevTools({ mode: 'detach' });
  });
  ...
  else mainWindow.loadFile(path.join(__dirname, '..', 'dist', 'public', 'index.html'));
...
ipcMain.handle('p2p:start', async (_event, options = {}) => { ensureDataDir(); loadManifests(); ensureTransport(options); return networkSummary(); });
ipcMain.handle('p2p:listFiles', async (_event, payload = {}) => {
  const query = String(payload.query || '').trim().toLowerCase();
  if (!query) return manifests;
  return manifests.filter((file) => [file.name, file.hash, file.rootHash].some((value) => String(value || '').toLowerCase().includes(query)));
});
...
ipcMain.handle('p2p:repair', async () => {
  const node = ensureTransport({});
  const report = manifests.flatMap((file) => (file.chunks || []).map((chunk) => {
    const replicas = Array.from(new Set([node.peerId, ...(chunk.replicas || [])]));
    return { file: file.name, rootHash: file.rootHash, chunkIndex: chunk.index, chunkHash: chunk.hash, healthyReplicas: replicas, targetReplicas: TARGET_REPLICAS, underReplicated: replicas.length < TARGET_REPLICAS };
  }));
...
app.whenReady().then(() => { ensureDataDir(); loadManifests(); ensureTransport({}); createMainWindow(); app.on('activate', () => { if (BrowserWindow.getAllWindows().length === 0) createMainWindow(); }); }).catch((error) => { console.error('Electron failed:', error); app.exit(1); });
app.on('before-quit', () => { persistManifests(); if (transportNode) transportNode.stop(); });
