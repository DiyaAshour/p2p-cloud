...
function readQueue() {
  ...
  if (!parsed || typeof parsed !== 'object') return { version: 1, updatedAt: nowIso(), items: [] };
  return { version: 1, updatedAt: parsed.updatedAt || nowIso(), items: Array.isArray(parsed.items) ? parsed.items : [] };
}
function writeQueue(queue) {
  const items = Array.isArray(queue.items) ? queue.items.slice(0, MAX_QUEUE_ITEMS) : [];
  writeJson(queuePath(), { version: 1, updatedAt: nowIso(), items });
...
function queueStats(queue = readQueue()) {
  const now = nowMs();
  const items = Array.isArray(queue.items) ? queue.items : [];
  return { queued: items.length, due: items.filter((i) => Number(i.nextAttemptAt || 0) <= now).length, targetReplicas: TARGET_REPLICAS, queuePath: queuePath(), lastStatus };
}
...
function scorePeer(peer = {}) { const health = peer.health || {}; const pressure = peer.pressure || {}; const storage = peer.remoteStorage || {}; return Number(health.score || 0) + bucketWeight(health.bucket) + (storage.acceptingChunks === true ? 10 : storage.acceptingChunks === false ? -80 : 0) - (pressure.overloaded ? 40 : 0); }
...
async function peerTargets(summary = {}, knownReplicas = []) {
  ...
  return mergePeers(summary.peers || [], healthPeers)
...
function putChunkWithAck({ peer, chunk, fromPeerId }) {
  return new Promise((resolve) => {
    ...
    socket.on('message', (raw) => { let message; try { message = JSON.parse(raw.toString()); } catch { return; } if (message.type !== 'chunk:stored-ack') return; if (message.payload?.ackTo && message.payload.ackTo !== messageId) return; if (message.payload?.chunkHash && message.payload.chunkHash !== chunk.hash) return; if (message.payload?.ok === false) return finish({ ok: false, peerId: peer.peerId, error: message.payload?.error || 'chunk rejected' }); finish({ ok: true, peerId: peer.peerId, score: scorePeer(peer), bucket: peer.health?.bucket || 'unknown' }); });
...
function findQueuedChunk(manifests, item) {
  const manifestIndex = manifests.findIndex((m) => m?.hash === item.fileHash);
  if (manifestIndex < 0) return null;
  const manifest = manifests[manifestIndex];
  const chunks = Array.isArray(manifest.chunks) ? manifest.chunks : [];
  const chunkIndex = chunks.findIndex((c) => c?.hash === item.chunkHash || Number(c?.index) === Number(item.chunkIndex));
  if (chunkIndex < 0) return null;
  return { manifestIndex, chunkIndex, manifest, chunk: chunks[chunkIndex] };
}
...
async function processQueueItem(item, summary) {
  ...
  const targets = await peerTargets(summary, Array.from(replicas));
  const failed = [];
  ...
  try {
    ...
    for (const item of due) {
      ...
      if (result.done || result.removed) remaining.delete(item.key);
        ...
        remaining.set(item.key, { ...item, attempts, lastError: result.error || 'not-enough-peers', lastAttemptAt: nowIso(), nextAttemptAt: nowMs() + backoffMs(attempts), updatedAt: nowIso() });
      }
    }
...
function installRepairQueue() {
  ...
  console.log('[repair-queue] installed', { targetReplicas: TARGET_REPLICAS, maxItemsPerRun: MAX_ITEMS_PER_RUN, intervalMs: RUN_INTERVAL_MS, queuePath: queuePath() });
}
