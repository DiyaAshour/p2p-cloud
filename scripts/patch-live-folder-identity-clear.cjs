...
// Add a small runtime guard if the refresh block has not been patched yet.
if (!s.includes('[folderStorageKey]);') && !s.includes('setFolderParents({});\n    setActiveFolder(ALL_FILES);')) {
  console.warn('[patch-live-folder-identity-clear] identity clear guard could not be verified');
}

if (changed) {
  fs.writeFileSync(p, s, 'utf8');
  console.log('[patch-live-folder-identity-clear] folders clear on logout and identity switch');
} else {
