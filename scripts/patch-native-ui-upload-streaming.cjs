...
// Replace the browser file input with a clear native-streaming message.
write(s.replace(/<Input type="file" multiple onChange=\{handleFileSelect\} disabled=\{!walletConnected \|\| busy\} \/>/g, '<div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4 text-sm text-zinc-300">Large-file safe mode is enabled. Press Encrypt & Upload to choose files with the Windows picker. Files are streamed from disk instead of loaded into browser RAM.</div>'));
...
write(s.replace(/disabled=\{!walletConnected \|\| busy \|\| !selectedFiles\.length \|\| uploadWouldExceedQuota\}/g, 'disabled={!walletConnected || busy}'));
write(s.replace(/disabled=\{!walletConnected \|\| busy \|\| !selectedFiles\.length\}/g, 'disabled={!walletConnected || busy}'));
