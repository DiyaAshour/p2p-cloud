...
const helperBlock = String.raw`function sanitizeFolderManifest(folder = {}) { Object.assign(folder, { kind: 'folder', isFolder: true, isEncrypted: false, visibility: 'private', isPublic: false, size: 0, storedSize: 0, totalChunks: 0, chunkSize: 0, chunks: [], encryption: null, replicas: Array.isArray(folder.replicas) ? folder.replicas : [] }); return folder; }
...
function removeBrokenSanitizeFolderName(source) {
  const marker = "function sanitizeFolderName(name = '')";
  let next = source;
  let index = next.indexOf(marker);
  while (index !== -1) {
    const followingFunction = next.indexOf('\nfunction folderIdFromName', index);
    if (followingFunction === -1) break;
    const replacement = "function sanitizeFolderName(name = '') { const clean = String(name || '').trim().replace(/\\s+/g, ' '); if (!clean) throw new Error('Folder name is required'); if (clean.length > 80) throw new Error('Folder name is too long'); if (['all files', 'uncategorized'].includes(clean.toLowerCase())) throw new Error('Reserved folder name'); return clean; }";
    next = next.slice(0, index) + replacement + next.slice(followingFunction);
    index = next.indexOf(marker, index + replacement.length);
  }
  return next;
}
