...
function replaceAny(source, replacements) {
  for (const [from, to] of replacements) if (source.includes(from)) return source.replace(from, to);
  return source;
}

function forceWalletInput(source) {
  let s = source;
  if (!s.includes('const [walletAddressInput, setWalletAddressInput]')) {
    s = s.replace(
      '  const [drivePassword, setDrivePassword] = useState("");',
      '  const [drivePassword, setDrivePassword] = useState("");\n  const [walletAddressInput, setWalletAddressInput] = useState(" ".trim());'
    );
  }
  const start = s.indexOf('  const connectWallet = () => run(async () => {');
  const endMarker = '\n  const disconnectWallet';
  const end = start >= 0 ? s.indexOf(endMarker, start) : -1;
  if (start >= 0 && end > start) {
    s = s.slice(0, start) + '  const connectWallet = () => run(async () => {\n    const address = walletAddressInput.trim();\n    if (!address) throw new Error("Enter wallet address first.");\n    setWallet(await api.invoke<WalletState>("wallet:connect", { address }));\n    setWalletAddressInput("");\n    await refresh();\n  });' + s.slice(end);
  }
  s = s.replace(
    '<Button onClick={connectWallet} disabled={busy}><Wallet className="size-4" />Connect Wallet</Button>',
    '<div className="space-y-2"><Input value={walletAddressInput} onChange={(event) => setWalletAddressInput(event.target.value)} placeholder="Wallet address 0x..." /><Button className="w-full" onClick={connectWallet} disabled={busy}><Wallet className="size-4" />Connect Wallet</Button></div>'
  );
  return s;
}
...
if (live.includes('window.prompt(')) console.warn('[patch-live-check-errors] warning: window.prompt still exists in NativeP2PAppLive');
else console.log('[patch-live-check-errors] no window.prompt remains in NativeP2PAppLive');
...
console.log('[patch-live-check-errors] fixed wallet input UI, seed account UI, no-prompt wallet connect, and live patches');
