...
for (const file of ['electron/main.js', 'electron/main-stable.js']) {
  ...
  m = m.replace('function encryptPrivateBuffer(plainBuffer, ownerWallet = activeWallet(), drivePassword) {', 'function encryptPrivateBuffer(plainBuffer, ownerWallet = activeIdentity(), drivePassword) {');
