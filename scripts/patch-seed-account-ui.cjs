...
src = replaceOnce(src,
  ...
  '  const connectWallet = () => run(async () => { const address = window.prompt("Wallet address 0x...")?.trim(); if (!address) return; setGeneratedSeed(""); setWallet(await api.invoke<WalletState>("wallet:connect", { address })); await refresh(); });\n  const disconnectWallet = () => run(async () => { setGeneratedSeed(""); setWallet(await api.invoke<WalletState>("wallet:disconnect")); await refresh(); });\n  const createSeedAccount = () => run(async () => { const username = seedUsername.trim(); const password = seedPassword.trim(); if (!username || !password) throw new Error("Username and password are required."); const result = await api.invoke<SeedAuthResult>("seed:create", { username, password }); setWallet(result); setGeneratedSeed(result.seed || ""); setSeedRecovery(""); await refresh(); toast.success("Seed Account created. Save your recovery seed now."); });\n  const loginSeedAccount = () => run(async () => { const username = seedUsername.trim(); const password = seedPassword.trim(); if (!username || !password) throw new Error("Username and password are required."); const result = await api.invoke<SeedAuthResult>("seed:login", { username, password }); setGeneratedSeed(""); setWallet(result); await refresh(); toast.success("Signed in with Seed Account"); });\n  const recoverSeedAccount = () => run(async () => { const username = seedUsername.trim(); const password = seedPassword.trim(); const seed = seedRecovery.trim(); if (!username || !password || !seed) throw new Error("Username, new password, and recovery seed are required."); const result = await api.invoke<SeedAuthResult>("seed:recover", { username, password, seed }); setGeneratedSeed(""); setWallet(result); await refresh(); toast.success("Seed Account recovered and password reset"); });\n',
...
const accountCardReplacement = '<Card className="rounded-2xl border-zinc-800 bg-zinc-900"><CardContent className="space-y-4 p-5"><p className="text-sm text-zinc-400">Identity</p><p className="truncate font-medium">{identityLabel}</p><p className="text-xs text-zinc-500">{wallet?.authMode === "seed" ? short(wallet?.seedFingerprint || wallet?.address || "") : walletConnected ? "Wallet identity" : "No identity connected"}</p>{walletConnected ? <Button variant="outline" onClick={disconnectWallet} disabled={busy}>Disconnect</Button> : <Button onClick={connectWallet} disabled={busy}><Wallet className="size-4" />Connect Wallet</Button>}</CardContent></Card><Card className="rounded-2xl border-zinc-800 bg-zinc-900"><CardHeader><CardTitle className="text-base"><KeyRound className="mr-2 inline size-4" />Seed Account</CardTitle></CardHeader><CardContent className="space-y-3"><div className="grid grid-cols-3 gap-1 rounded-xl bg-zinc-950 p-1 text-xs"><button type="button" onClick={() => setSeedMode("login")} className={`rounded-lg px-2 py-2 ${seedMode === "login" ? "bg-zinc-800" : "text-zinc-500"}`}>Login</button><button type="button" onClick={() => setSeedMode("create")} className={`rounded-lg px-2 py-2 ${seedMode === "create" ? "bg-zinc-800" : "text-zinc-500"}`}>Create</button><button type="button" onClick={() => setSeedMode("recover")} className={`rounded-lg px-2 py-2 ${seedMode === "recover" ? "bg-zinc-800" : "text-zinc-500"}`}>Recover</button></div><Input value={seedUsername} onChange={(e) => setSeedUsername(e.target.value)} placeholder="Username" /><Input type="password" value={seedPassword} onChange={(e) => setSeedPassword(e.target.value)} placeholder={seedMode === "recover" ? "New password" : "Password"} />{seedMode === "recover" && <textarea value={seedRecovery} onChange={(e) => setSeedRecovery(e.target.value)} placeholder="Paste recovery seed" className="min-h-24 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm outline-none" />}{seedMode === "create" && <p className="text-xs text-amber-300">After create, save the recovery seed. Without it, forgotten passwords cannot be recovered.</p>}{generatedSeed && <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3"><p className="text-xs font-medium text-amber-200">Recovery seed — save it now</p><p className="mt-2 break-all rounded-lg bg-zinc-950 p-2 text-xs text-zinc-200">{generatedSeed}</p><Button className="mt-2 w-full" size="sm" variant="outline" onClick={() => navigator.clipboard.writeText(generatedSeed).then(() => toast.success("Seed copied"))}>Copy seed</Button></div>}<Button className="w-full" variant={seedMode === "login" ? "default" : "outline"} onClick={seedMode === "create" ? createSeedAccount : seedMode === "recover" ? recoverSeedAccount : loginSeedAccount} disabled={busy}>{seedMode === "create" ? "Create Seed Account" : seedMode === "recover" ? "Recover Account" : "Login with Seed"}</Button></CardContent></Card>';
...
write(appPath, src);
console.log('[seed-account-ui] Seed Account UI patched into NativeP2PAppLive.tsx');
