...
function warnIfIncludes(source, needle, label) {
  if (source.includes(needle)) warn(label || `Found ${needle}`);
  else pass(label || `Did not find ${needle}`);
}
...
function runCommand(command, args, label) {
  ...
  if (result.status === 0) {
    pass(`${label || display} passed`);
  } else {
    fail(`${label || display} failed with code ${result.status}\n${result.stdout || ''}${result.stderr || ''}`.trim());
  }
}
...
function hasHandler(source, channel) {
  return source.includes(`ipcMain.handle('${channel}'`) || source.includes(`ipcMain.handle("${channel}"`);
}
...
function checkPackage() {
  ...
  const startupScripts = ['dev', 'electron:dev', 'electron:dev:fast', 'start', 'start:fast'];
  for (const name of startupScripts) {
    const value = String(scripts[name] || '');
    assert(!/patch-[\w-]+\.cjs/.test(value), `${name} does not call patch scripts`);
  }

  const legacyPatchRefs = Object.entries(scripts).filter(([, value]) => /patch-[\w-]+\.cjs/.test(String(value)));
  if (legacyPatchRefs.length) {
    warn(`Legacy patch script references still exist in non-startup scripts: ${legacyPatchRefs.map(([k]) => k).join(', ')}`);
  } else {
    pass('No patch script references remain in package scripts');
  }
}
...
function checkRequiredFiles() {
  ...
  for (const file of [
    ...
    'electron/core/identity.js',
    'electron/core/storage-paths.js',
    'electron/core/storage-json.js',
    ...
    'electron/tombstone-sync-pull-override.js',
    ...
    'electron/seed-auth-cooldown-ipc.js',
...
function checkCoreModules() {
  ...
  for (const needle of ['PLANS', 'CHUNK_SIZE_BYTES', 'TARGET_REPLICAS', 'FOLDER_MANIFEST_KIND', 'UI_PREFS_MANIFEST_KIND']) {
    assertIncludes(config, needle, `config exports/defines ${needle}`);
  }
  for (const needle of ['normalizeIdentity', 'activeIdentity', 'assertVerified']) {
    assertIncludes(identity, needle, `identity has ${needle}`);
  }
  for (const needle of ['readWallet', 'readManifests', 'writeManifests', 'writeWallet']) {
    assertIncludes(storageJson, needle, `storage-json has ${needle}`);
  }
  for (const needle of ['storageRoot', 'walletPath', 'manifestsPath']) {
    assertIncludes(storagePaths, needle, `storage-paths has ${needle}`);
  }
}
...
function checkIpcModules() {
  ...
  for (const [file, needles] of Object.entries(modules)) {
    ...
    for (const needle of needles) {
      assertIncludes(source, needle, `${file} includes ${needle}`);
    }
  }
}
...
function checkTransferAndDelete() {
  ...
  for (const needle of ['startTransfer', 'updateTransfer', 'finishTransfer', 'failTransfer', 'throwIfTransferCancelled']) {
    assertIncludes(transferState + upload, needle, `transfer progress has ${needle}`);
  }
  assertIncludes(transferState + upload, 'p2p:cancelTransfer', 'cancel transfer IPC exists');
...
function checkRuntimeCoreHandlers() {
  ...
  for (const channel of ['p2p:start', 'p2p:networkSummary', 'p2p:listFiles', 'p2p:uploadFiles']) {
    assert(hasHandler(combined, channel), `main/main-stable define core handler ${channel}`);
  }
}
...
function checkClientPurity() {
  ...
  const forbidden = [
    { pattern: /\bfetch\s*\(/, label: 'browser fetch()' },
    { pattern: /\baxios\b/, label: 'axios' },
    { pattern: /\btrpc\b/i, label: 'TRPC' },
    { pattern: /VITE_P2P_API_BASE_URL/, label: 'VITE_P2P_API_BASE_URL' },
    { pattern: /P2P API is not reachable/i, label: 'old browser API error text' },
  ];

  for (const file of files) {
    const source = read(file);
    for (const item of forbidden) {
      if (item.pattern.test(source)) warn(`${file} contains ${item.label}; verify it is not used for runtime data path`);
    }
  }
...
function checkLegacyPatchesNotStartup() {
  ...
  const pkg = readJson('package.json');
  if (!pkg) return;
  const startupValues = ['dev', 'electron:dev', 'electron:dev:fast', 'start', 'start:fast', 'renderer:build']
    .map((key) => String(pkg.scripts?.[key] || ''))
    .join('\n');
...
function checkVerifyRuntime() {
  section('verify-runtime');
  const output = runNodeScript('scripts/verify-runtime.cjs');
...
function deepChecks() {
  ...
  runCommand('pnpm', ['run', 'build:fast'], 'pnpm run build:fast');
}
...
checkCoreModules();
checkIpcModules();
checkTransferAndDelete();
checkRuntimeCoreHandlers();
checkClientPurity();
checkLegacyPatchesNotStartup();
checkVerifyRuntime();
deepChecks();
printSummary();
