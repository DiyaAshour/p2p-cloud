...
export default function NativeP2PAppLive() {
  ...
  const visibleFiles = useMemo(() => {
    ...
    return files.filter((file) => {
      ...
      const queryOk = !q || [file.name, file.hash, file.rootHash, folder, folderPath(folder)].some((x) => String(x || "").toLowerCase().includes(q));
  ...
  const proof = (file: P2PFile) => run(async () => {
    const r = await api.invoke<{ proof: unknown }>("p2p:prepareProof", { hash: file.hash });
    await navigator.clipboard.writeText(JSON.stringify(r.proof, null, 2));
